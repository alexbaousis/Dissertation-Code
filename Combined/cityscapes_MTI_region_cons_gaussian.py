...
parser.add_argument('--out', default='./results/cityscapes/MTI-Net', help='Directory to output the result')
...
file_list = []
file_list.append('cityscapes_MTI_region_cons_gaussian.py')
...
model = get_MTI(tasks, class_nb=7)
model = model.cuda()
...
#    print('({batch}/{size}) | LossS: {loss_s:.4f} | LossD: {loss_d:.4f} | LossN: {loss_n:.4f} | Ws: {ws:.4f} | Wd: {wd:.4f}| Wn: {wn:.4f} | CTL: {ctl:.4f} | CW: {cw:.2f}'.format(
    ...
    if evaluate:
        ...
        with torch.no_grad():  # operations inside don't track history
            ...
            # store metrics
            avg_cost[index, 7], avg_cost[index, 8] = conf_mat.get_metrics()
